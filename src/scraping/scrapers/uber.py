...
class UberScraper(BaseScraper):
    ...
    async def afetch(self) -> list[Job]:
        async with self.make_fetcher() as fetch:
            ...
            for page in range(2, total_pages + 1):
                ...
                if page_total != total:
                    raise ScraperError(
                        f"Uber careers total changed during pagination: {total} -> {page_total}"
                    )
        ...
        if len(jobs_by_id) != total:
            raise ScraperError(
                f"Uber careers advertised {total} jobs but returned {len(jobs_by_id)} unique IDs"
            )
    ...
    def _parse_job(self, item: Any) -> Job:
        ...
        return Job(
            ...
            description=(
                _html_to_text(item.get("Description")) if self.include_descriptions else None
            ),
