...
from src.scraping.models import ATSType, EmploymentType, Job, SalaryPeriod
...
class BundesagenturScraper(BaseScraper):
    ...
    async def _collect_verified_cover(
        ...
        *,
        ...
        if total > MAX_COVER_TOTAL:
            raise ScraperError(
                f"Bundesagentur refuses an unbounded cover of {total} jobs for params={base_params}"
            )
        ...
        for _attempt in range(MAX_COVER_ATTEMPTS):
            ...
            observations.append(f"{start_total}/{len(references)}/{end_total}")
            if complete_pass:
        ...
        *,
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if r.status_code == 200:
                try:
                    ...
                    if not isinstance(payload, dict):
                        last_exc = ScraperError("Bundesagentur returned a non-object payload")
                    else:
                        try:
            ...
            if r.status_code == 400:
                raise ScraperError(f"Bundesagentur rejected query {params}: {r.text[:120]}")
    ...
    def _parse(self, item: dict[str, Any]) -> Job | None:
        ...
        title = str(
            item.get("stellenangebotsTitel") or item.get("titel") or item.get("hauptberuf") or ""
        ).strip()
        ...
        company = (
            str(item.get("firma") or item.get("arbeitgeber") or "Bundesagentur").strip()
            or "Bundesagentur"
        )
...
def _result_total(payload: dict[str, Any]) -> int:
    ...
    try:
        ...
        raise ScraperError("Bundesagentur response is missing a valid maxErgebnisse") from exc
    if total < 0:
...
def _employment_details(
    ...
    if legacy:
        ...
        if legacy_employment_type is None and str(item.get("befristung") or "") == "2":
...
def _format_locations(values: list[dict[str, Any]]) -> str | None:
    locations = [location for value in values if (location := _format_location(value)) is not None]
