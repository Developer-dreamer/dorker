...
def evaluate_location_relevance(location_str: str) -> str:
    ...
    global_markers = [
        ...
        "apac",
        "utc",
    ]  # maybe should add 'amer' but not for me obviously
    if any(re.search(r"\b" + re.escape(w) + r"\b", location) for w in global_markers) or re.search(
        r"\b(eu|world)\b", location
    ):
        return "KEEP_GLOBAL"
    ...
    if (
        any(w in location for w in us_specific_words)
        or re.search(r"\b(us|usa|u\.s|sf|nyc|la)\b", location)
        or location.endswith("-united-states")
    ):
        return "EXCLUDE_US"
    ...
    if re.search(
        r"\b(ind|bra|bgr|arg|deu|aus|can|usa|phl|gbr|fra|esp|mex|col|prt|tha|are|nga|gtm)\b",
        location,
    ):
        return "EXCLUDE_OTHER_COUNTRY"
